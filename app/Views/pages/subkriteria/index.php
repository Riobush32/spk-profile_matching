        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5 text-center">
                ...
                <div class="flex gap-3 w-full justify-center items-center">
                    <form id="deleteForm" action="/sub-kriteria/delete" method="POST">
                        <?= csrf_field() ?>
...
const tableData = subkriteriaData.map(subkriteria => [
    subkriteria.nama_kriteria, // Misalnya kolom 'nama_kriteria'
    subkriteria.nama_subkriteria,
    subkriteria.nilai,
    `<a href="/sub-kriteria/edit/${subkriteria.id}" class="text-white bg-gradient-to-r from-teal-400 via-teal-500 to-teal-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-teal-300 dark:focus:ring-teal-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2" data-id="${subkriteria.id}"><i class="fa-solid fa-pen-to-square"></i></a>` +
    // Tombol Edit
    `<button data-modal-target="popup-modal" data-modal-toggle="popup-modal" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2" data-id="${subkriteria.id}"><i class="fa-solid fa-trash"></i></button>` // Tombol Delete
]);

if (document.getElementById("search-table") && typeof simpleDatatables.DataTable !== 'undefined') {
    const dataTable = new simpleDatatables.DataTable("#search-table", {
        data: {
            headings: ["Nama Kriteria", "Nama SubKriteria", "Nilai",
                "Action"
            ], // Header kolom
