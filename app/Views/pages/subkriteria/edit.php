...
<div class="absolute top-20 right-20" x-data="{ flash_messege: true }">
    <div x-init="setTimeout(() => flash_messege = false, 3000)" x-show="flash_messege"
        ...
        x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-300"
        x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90"
        class="flex items-center p-4 mb-4 text-sm text-amber-800 rounded-lg bg-amber-50 dark:bg-gray-800 dark:text-amber-400"
        role="alert">
        <svg class="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
            fill="currentColor" viewBox="0 0 20 20">
            <path
        ...
        <span class="sr-only">Info</span>
        <div>
            <span class="font-medium">eror alert!</span> <?= session()->getFlashdata('eror') ?>
        </div>
    </div>
</div>
<?php endif; ?>
...
<div class="w-full flex justify-start items-center ">
    <form action="/sub-kriteria/update/<?=$subkriteria['id']?>" method="post"
        class="w-96 p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-5 dark:text-white">
        <?= csrf_field()?>
        <input type="hidden" name="_method" value="PATCH">
        <div class="mt-3">
            <label for="small" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jenis
                Kriteria</label>
            <select id="small" name="kriteria_id" required autofocus
                ...
                <option selected value="<?= $subkriteria['kriteria_id']?>"><?= $subkriteria['nama_kriteria']?></option>
        ...
        <div class="mt-3">
            <label for="small-input" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                SubKriteria</label>
            <input type="text" id="small-input" name="nama_subkriteria" required
        ...
        <div class="mt-3">
            ...
            <input type="number" step="0.01" id="small-input" name="nilai" required value="<?= $subkriteria['nilai']?>"
                class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 invalid:focus:ring-rose-500 invalid:focus:border-rose-500">
        </div>
